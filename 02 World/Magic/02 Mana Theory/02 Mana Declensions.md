## Primitive Declensions

| Scientific Name | Common Names                              | Primitive Operation                                                                         |
| --------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------- |
| **Locative**    | Position, Translation, Teleportation      | Change an object's position in space.                                                       |
| **Kinetic**     | Motion, Force, Heat                       | Change the momentum distribution of matter (uniform or microscopic).                        |
| **Massic**      | Mass, Weight, Gravity                     | Modify an object's inertial and gravitational mass.                                         |
| **Volumic**     | Pressure, Density, Compression, Expansion | Change local volume and spacing between matter while preserving mass.                       |
| **Radiant**     | Light, Illusion, Cloaking                 | Emit, absorb, redirect or otherwise manipulate photons.                                     |
| **Galvanic**    | Lightning, Magnetism                      | Modify electric charge and charge distribution.                                             |
| **Temporal**    | Acceleration, Preservation, Stasis        | Modify the local rate at which time progresses.                                             |
| **Resonant**    | Sympathy, Linking, Attunement             | Establish, strengthen or weaken persistent interactions between otherwise separate systems. |
| **Cohesive**    | Structure, Shaping, Binding               | Alter adjacency and continuity relationships within matter.                                 |
| **Mnemonic**    | Memory, Recording, Engraving              | Encode, erase or modify information stored in physical systems.                             |
| **Ontic**       | Identity, Essence, Classification, Soul   | Modify what an object fundamentally _is_ according to magical law.                          |

## Declension Classifications

|Declension|Operational Domain|Reality Layer|Primary Property|Typical Scale|Complexity|Typical Reversibility|Relative Complexity|
|---|---|---|---|---|---|---|---|
|**Locative**|State|Physical|Position|Object|Low|Complete|★☆☆☆☆|
|**Kinetic**|State|Physical|Momentum|Particle → Object|Low|High|★☆☆☆☆|
|**Massic**|State|Physical|Mass|Object|Medium|High|★★☆☆☆|
|**Volumic**|State|Physical|Volume / Pressure|Local region|Medium|High|★★☆☆☆|
|**Radiant**|State|Physical|Photon field|Field|Medium|Complete|★★☆☆☆|
|**Galvanic**|State|Physical|Charge|Field|Medium|Complete|★★☆☆☆|
|**Temporal**|State*|Physical*|Time rate|Local region|High|Moderate|★★★☆☆|
|**Resonant**|Relational|Emergent|Coupling|Multiple systems|High|High|★★★☆☆|
|**Cohesive**|Organizational|Emergent|Connectivity|Material structure|Very High|Difficult|★★★★☆|
|**Mnemonic**|Organizational|Emergent|Information|Pattern|Very High|Variable|★★★★☆|
|**Ontic**|Semantic|Metaphysical|Identity|Entity|Extreme|Rarely|★★★★★|

*\* Temporal Declension classification is a hotly debated topic*